...
export function Header({ onSearchOpen }: { onSearchOpen?: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-200">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-3 sm:gap-6">
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            className="md:hidden p-1.5 rounded-md text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            {mobileNavOpen ? (
      ...
      {mobileNavOpen && (
        <nav
          className="md:hidden border-t border-slate-200 dark:border-slate-800 px-4 py-2 flex flex-col gap-1 bg-white dark:bg-slate-900"
          aria-label="Mobile navigation"
        >
          <NavLink to="/modules" className={mobileLinkClass}>Modules</NavLink>
          <NavLink to="/simulations" className={mobileLinkClass}>Simulations</NavLink>
          <NavLink to="/reference" className={mobileLinkClass}>Reference</NavLink>
        </nav>
      )}
    </header>
  );
